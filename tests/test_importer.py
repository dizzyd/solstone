...
def test_importer_audio_transcribe(tmp_path, monkeypatch):
    ...
    mock_revai_response = {
        "monologues": [
            ...
            },
            {
                "speaker": 1,
                "elements": [
                    {"type": "text", "value": "Second", "ts": 310.0, "confidence": 0.88},  # After 5 minutes
    ...
    with patch("think.importer.transcribe_file") as mock_transcribe:
        ...
        monkeypatch.setattr(
            "sys.argv",
            ["think-importer", str(audio_file), "--timestamp", "20240101_120000", "--hear", "true", "--see", "false", "--split", "false"],
        )
        mod.main()
