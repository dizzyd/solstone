...
def gather_files(day: datetime, day_dirs: Dict[str, str]) -> List[str]:
    ...
    for i in range(8):
        ...
        if not dir_path:
            continue
        kg_pattern = os.path.join(dir_path, "kg*.md")
...
def process_day(day_str: str, day_dirs: Dict[str, str], force: bool) -> None:
    ...
    if not files:
        print(f"No kg.md files for {day_str}")
        return

    print(f"Processing {day_str}:")
    print(f"  Found {len(files)} kg files from 8-day window")
    for file in files:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Merge kg files from a rolling 8-day window and generate entities.md"
    )
